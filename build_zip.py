...
パッケージに含めるのは *.py と blender_manifest.toml のみ (ガイドラインの
標準構成に合わせる。ライセンスはマニフェストの SPDX 宣言で扱われるため
LICENSE ファイルも同梱しない。ドキュメント・テスト・ツール類も含めない)。
"""
