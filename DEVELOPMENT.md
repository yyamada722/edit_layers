...
配布パッケージに入るのは *.py + blender_manifest.toml のみ
(ライセンスはマニフェストの SPDX 宣言で扱う)
(ドキュメント・テスト・ツールはリポジトリ管理のみで、パッケージには含めない)。

- `__init__.py` — エントリポイント (クラス登録・ハンドラ管理のみ)
- `common.py` — 定数とセッション状態 (モジュール横断キャッシュ)
- `i18n.py` — UI 翻訳 (英語ソース + 日本語辞書)
- `core.py` — 差分エンジン (永続 ID / スナップショット / 差分 / 適用)
- `stack.py` — スタック運用 (ブランチ / 再構築 / 状態検出 / ガード)
- `props.py` — プロパティグループ
- `operators.py` — オペレータ
- `ui.py` — パネル・リスト・オーバーレイ (ヘルプボタンはオンライン URL を開く)
- `handlers.py` — シェイプキーロック等のアプリケーションハンドラ
